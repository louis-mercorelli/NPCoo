...
final class CEHDebug {
    ...
    static int handleDebugScreenToggle(CommandContext<CommandSourceStack> context, boolean enabled) {
        ...
        context.getSource().sendSuccess(
            () -> Component.literal(
                "SteveAI debug " + (enabled ? "enabled" : "disabled")
                + " (screen messages and server heartbeat stream)."
            ),
            false
        );
        CommandEvents.LOGGER.info(
            com.sai.NpcooLog.tag("SteveAI debug set to {} by {} (screenDebug={}, heartbeat={})"),
            enabled,
            context.getSource().getTextName(),
            CommandEvents.screenDebugEnabled,
            ServerLoad.isHeartbeatLoggingEnabled()
        );
        return 1;
    }
}
